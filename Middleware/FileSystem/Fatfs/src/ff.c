...
)
{
	...
	do {
		...
		if (wc) {	/* Last character has not been processed */
			wc = ffxx_wtoupper(uc);		/* Convert it to upper case */
			if (i >= _MAX_LFN || wc != ffxx_wtoupper(lfnbuf[i++]))	/* Compare it */
...
#if !_LFN_UNICODE
				w = ffxx_convert(w, 0);		/* Unicode -> OEM */
...
)
{
	...
	for (;;) {
		...
		w = ffxx_convert(w, 1);			/* Convert ANSI/OEM to Unicode */
...
#else
			w = ffxx_convert(ffxx_wtoupper(w), 0);	/* Upper converted Unicode -> OEM code */
#endif
...
extern WCHAR ffxx_convert (	/* Converted code, 0 means conversion error */
...
)
{
	...
	if (sl) {	/* Create volume label in directory form */
		...
		do {
#if _USE_LFN && _LFN_UNICODE
			w = ff_convert(ffxx_wtoupper(label[i++]), 0);
#else
			w = (BYTE)label[i++];
			if (IsDBCS1(w))
				w = (j < 10 && i < sl && IsDBCS2(label[i])) ? w << 8 | (BYTE)label[i++] : 0;
#if _USE_LFN
			w = ffxx_convert(ffxx_wtoupper(ffxx_convert(w, 1)), 0);
#else
